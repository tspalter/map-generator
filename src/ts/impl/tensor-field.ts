...
import { BasisField, Grid, Radial } from './basis-field';
...
export class TensorField {
  ...
  getCentrePoints(): Vector[] {
    return this.basisFields.map((field) => field.center);
  }
  ...
  samplePoint(point: Vector): Tensor {
    ...
    if (this.noiseParams.globalNoise) {
      tensorAcc.rotate(
        this.getRotationalNoise(point, this.noiseParams.noiseSizeGlobal, this.noiseParams.noiseAngleGlobal),
      );
    }
